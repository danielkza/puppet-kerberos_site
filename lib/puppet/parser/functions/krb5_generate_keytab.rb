...
module Puppet::Parser::Functions.newfunction(:krb5_generate_keytab,
...
do |args|
  ...
  if (! princ_names.is_a?(Enumerable) ||
    ...
    raise Puppet::ParseError,
      'Invalid parameter: must be a non-empty array of strings'
  end
  ...
  keytab_dirs = {:principals => 'principals', :host => certname}
  keytab_dirs.each do |k, v|
    v.replace(File.join(keytab_dir, v))
    File.mkdir(v, :mode => 0700) unless File.directory?(v)
  }
  ...
  def keytab_needs_regen?(path, name = nil, mtime)
    if ! File.exists?(path) || File.mtime(path) < mtime
      ...
      Puppet.info("Keytab for #{name} is #{status}, regenerating") if name
      true
    else
      false
    end
